...
// ListDirectoriesOpenAPI 使用 115 OpenAPI AccessToken 获取目录列表。
func (h *Web115CookieHandler) ListDirectoriesOpenAPI(c *gin.Context) {
	userIDVal, exists := c.Get("user_id")
	if !exists {
		h.error(c, http.StatusUnauthorized, 401, "用户未认证")
		return
	...
	if storage.StorageType != model.StorageType115Open {
		h.error(c, http.StatusBadRequest, 400, "所选云存储不是 115 OpenAPI 账号")
		return
	}
	if storage.Status != model.StatusActive {
		h.error(c, http.StatusBadRequest, 400, "115 OpenAPI 账号不可用")
		return
	}
	if strings.TrimSpace(storage.AccessToken) == "" {
		h.error(c, http.StatusBadRequest, 400, "115 OpenAPI AccessToken 为空")
		return
	}
	...
	listResp, err := h.web115Svc.GetDirectoriesWithOpenAPI(
		c.Request.Context(), storage.AccessToken, cid, req.Offset, req.Limit,
	)
	if err != nil {
		h.error(c, http.StatusBadRequest, 400, "通过 115 OpenAPI 获取目录列表失败")
		return
	}
